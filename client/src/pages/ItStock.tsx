...
export default function ItStocks() {
  ...
  const handleExport = async () => {
    try {
      const response = await api.get(`/it-inventory/export/`, {
        params: {
          from: exportFrom,
          to: exportTo,
        },
        responseType: "blob", // Crucial for binary downloads
      });

      // FIX: Pass response.data directly because it's already a Blob
      const url = window.URL.createObjectURL(response.data);

      const link = document.createElement("a");
      link.href = url;

      // Optional fallback string just in case item_name is undefined
      const fileName = `IT_Stock_Report from ${exportFrom} to ${exportTo}.xlsx`;
      ...
      link.remove();
      window.URL.revokeObjectURL(url); // Free up browser memory allocation
